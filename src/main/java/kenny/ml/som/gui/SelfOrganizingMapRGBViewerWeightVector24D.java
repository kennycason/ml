...
public class SelfOrganizingMapRGBViewerWeightVector24D extends Canvas {
	...
	public static void main(String argv[]) {
		SelfOrganizingMapRGBViewerWeightVector24D somv = new SelfOrganizingMapRGBViewerWeightVector24D();
	...
	public SelfOrganizingMapRGBViewerWeightVector24D() {
		// create a frame
	...
	public void init() {
		...
		config.dimX = dim;
		config.dimY = dim;
		config.weightVectorDimension = 24;

		config.samples = new AbstractWeightVector[] {
				buildWeightVector(Color.BLACK.getRGB()), // black
				buildWeightVector(Color.PINK.getRGB()), // pink
				buildWeightVector(Color.WHITE.getRGB()), // white
				buildWeightVector(Color.BLUE.getRGB()), // blue
				buildWeightVector(Color.RED.getRGB()), // red
				buildWeightVector(Color.GREEN.getRGB()), // green
		};

		// config.samples = new AbstractWeightVector[] {
		// buildWeightVector(Color.BLACK.getRGB()), // black
		// buildWeightVector(Color.GRAY.getRGB()), // gray
		// buildWeightVector(Color.WHITE.getRGB()), // white
		// };
	...
	public void run() {
		...
		while (true) {
			if (t < 1.0) {
				...
				for (int i = 0; i < som.getSampleSize(); i++) {
					...
					AbstractMapLocation loc = som
							.calculateBestMatchingUnit(sample);

					// scale the neighbors according to t
					som.scaleNeighbors(loc, sample, t);
				}
				// System.out.println(som);
				// fw.write(som.toString());
				// increase t to decrease the number of neighbors and the
				// amount
		...
		State s = new State("save/2DRGB24D.som");
		s.save(som);
		// System.exit(0);
	...
	private void draw() {
		...
		for (int y = 0; y < config.dimY; y++) {
			for (int x = 0; x < config.dimX; x++) {
				drawSquare(x, y, som.getWeightVectors()[x][y], g);
			}
		}
	...
	private void drawSquare(int x, int y, WeightVector vector, Graphics2D graphics) {
		...
		// System.out.print(b + " ");
		graphics.setColor(new Color((int) b.value()));
