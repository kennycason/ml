...
public class SelfOrganizingMap3DTest {
	...
	public void Test() {
		...
		config.samples = new AbstractWeightVector[]{
				new WeightVector(0.0, 0.0, 0.0),
				new WeightVector(0.5, 0.5, 0.5),
				new WeightVector(0.9, 0.9, 0.9),
				};

		AbstractSelfOrganizingMap som = new SelfOrganizingMap3D(config);
		...
		System.out.println("Input (0.8, 0.8, 0.8)");
		WeightVector sample = new WeightVector(0.8, 0.8, 0.8);
		...
		System.out.println("Input (0.9, 0.9, 0.9)");
		sample = new WeightVector(0.9, 0.9, 0.9);
		processed = som.processInput(sample);
		for(int i = 0; i < config.samples.length; i++) {
			System.out.println(config.samples[i] + " => " + processed[i]);
		}
		
		System.out.println("Input (0.3, 0.5, 0.5)");
		sample = new WeightVector(0.3, 0.5, 0.5);
		processed = som.processInput(sample);
		for(int i = 0; i < config.samples.length; i++) {
			System.out.println(config.samples[i] + " => " + processed[i]);
		}
